...
2. Run [example.pig](https://github.com/tukichen/Social-Network-Graph-Analysis-on-AWS/blob/master/example.pig). example.pig is a Pig Latin script that loads and parses the billion triple dataset that we will use in this assignment into triples: (subject, predicate, object). Then it groups the triples by their object attribute and sorts them in descending order based on the count of tuple in each group.
